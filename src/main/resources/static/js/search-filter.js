function loadingSF() {
    ...
    if (localStorage.getItem('id-active-category') !== "") {
        $.ajax({
            ...
            success: function (dataBrand) {
                ...
                $("#li-brand" + localStorage.getItem('id-active-category')).append("<li><a style='cursor: pointer' onclick='viewByCategory(" +
                    localStorage.getItem('id-active-category') + ")'>" + "<strong>Всі види</strong></a></li>");
                localStorage.setItem('name-active-category', dataBrand[0].categories.name);
                var checked = '';
                var arr = localStorage.getItem('stringBrand').split(',');
                for (var j = 0; j < dataBrand.length; j++) {
                    for (var i = 0; i < arr.length; i++) {
                        if (dataBrand[j].id == arr[i]) {
                            checked = 'checked';
                        }
                    }

                    $("#li-brand" + localStorage.getItem('id-active-category')).append("<li><a style='cursor: pointer' onclick='viewByBrand(" +
                        dataBrand[j].id + ")'>" + dataBrand[j].brand + "</a></li>");
                    $(".checkbox-brand").append("<li><input " + checked + " type='checkbox' name='brand[]' value='" + dataBrand[j].id + "' " +
    ...
    if(localStorage.getItem('current-sorting') == '') {
        $.ajax({
            url: "/products/byBrands/" + localStorage.getItem('currentStartPrice') + '/' + localStorage.getItem('currentEndPrice') + sortString,
            type: "POST",
            contentType: "application/json",
...
var array = localStorage.getItem('stringBrand');
    $.ajax({
        url: "/products/byBrands/" + localStorage.getItem('currentStartPrice') + '/' + localStorage.getItem('currentEndPrice') + sortString,
        type: "POST",
        contentType: "application/json",
...
function loadNewPage(page) {
    ...
    $.ajax({
        ...
        success: function (products) {
            $("#block-tovar-grid li").remove();
            $("#block-tovar-list li").remove();
            console.log(products[0].brand.brand);
            for (var i = 0; i < products.length; i++) {
